...
float GLWidget::_gAdjustX = 0.0f;
float GLWidget::_gAdjustY = 0.0f;

QThread* GLWidget::_thread = new QThread();

AppFilter* GLWidget::_filter = new AppFilter();
  ...
  : QOpenGLWidget(parent)
{
  ...
  connect(_filter, SIGNAL(downPressed()), this, SLOT(adjustValueDown()));
  connect(_filter, SIGNAL(leftPressed()), this, SLOT(adjustValueLeft()));
  connect(_filter, SIGNAL(rightPressed()), this, SLOT(adjustValueRight()));
  connect(_filter, SIGNAL(upPressed()), this, SLOT(adjustValueUp()));
}
...
void GLWidget::adjustValueUp()
{
  _gAdjustY = -0.1f;
  glutRender();
}

void GLWidget::adjustValueDown()
{
  _gAdjustY = 0.1f;
  glutRender();
}

void GLWidget::adjustValueLeft()
{
  _gAdjustX = 0.1f;
  glutRender();
}

void GLWidget::adjustValueRight()
{
  _gAdjustX = -0.1f;
  glutRender();
}
...
void GLWidget::glutRender()
{
  ...
  scaleX += _gAdjustX/*0.01f*/;
  scaleY += _gAdjustY/*0.01f*/;

  Pipeline pipeLine;
  pipeLine.Rotate(scaleX, scaleY, 0.0f);
...
void GLWidget::startGlutLoop(GLWidget* me)
{
