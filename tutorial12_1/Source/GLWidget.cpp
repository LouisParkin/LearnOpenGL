...
void GLWidget::glutRender()
{
  ...
  Pipeline pipeLine;
  pipeLine.Rotate(scale / 2.0f, scale / 1.5f , 0.0f);
  pipeLine.WorldPos(sinf(scale/20), cosf(scale/20), fabs(cosf(scale/150) * 30) + 10 );
...
void GLWidget::startGlutLoop()
{
  ...
  /**
   * Greater FOV values yields an object further away from the camera
   */
